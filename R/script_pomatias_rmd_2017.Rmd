...
(NB: A "classical" .R script version is also available in the same folder as the RMarkdown file, for readers who'd rather use that. That version is much less commented, but both run the same analyses)
...
p1 <- newdata %>%
  ...
  ggplot() +
  stat_lineribbon(aes(x = Density, y = .value), .width = 0.95, point_interval = mean_hdi, alpha = 0.9, fill = "grey90") +
  ...
  scale_x_continuous("", sec.axis = sec_axis(name = expression(paste("Density (snails.", m^-2, " of release patch)")), trans = ~ . / (0.12 * 0.12))) +
...
p2 <- newdata %>%
  ...
  scale_x_continuous("Density (snails per box)", sec.axis = sec_axis(~ . / (0.12 * 0.12))) +
...
p3 <- newdata %>%
  ...
  mutate(Difference = post_act_ifdisp - post_act_ifres + mean(post_act_ifres)) %>%
  ...
  geom_hline(yintercept = mean(invlogit(newdata$post_act_ifres)), lty = 2, col = "darkgrey") +
  stat_eye(aes(x = Disp, y = value, alpha = Disp),
    .width = 0.95, interval_size = 1, point_size = 3, point_interval = mean_hdi,
  ) +
  stat_pointinterval(aes(x = Disp, y = value),
    .width = 0.95, interval_size = 1, point_size = 3, point_interval = mean_hdi,
  ) +
  scale_alpha_manual(values = c(0, 0.9, 0.9)) +
...
p4 <- newdata %>%
  ...
  mutate(Difference = post_disp_ifact - post_disp_ifstill + mean(post_disp_ifstill)) %>%
  ...
  geom_hline(yintercept = mean(invlogit(newdata$post_disp_ifstill)), lty = 2, col = "darkgrey") +
  stat_eye(aes(x = Active, y = value, alpha = Active),
    .width = 0.95, interval_size = 1, point_size = 3, point_interval = mean_hdi
  ) +
  stat_pointinterval(aes(x = Active, y = value),
    .width = 0.95, interval_size = 1, point_size = 3, point_interval = mean_hdi,
  ) +
  scale_alpha_manual(values = c(0, 0.9, 0.9)) +
...
## W the multiplicative comparison, for the text of the results
(invlogit(newdata$post_disp_ifact) / invlogit(newdata$post_disp_ifstill)) %>% mean_hdi()
(invlogit(newdata$post_act_ifdisp) / invlogit(newdata$post_act_ifres)) %>% mean_hdi()
```
...
# to override, re-run the model and re-save manually by selecting relevant code lines then knit (or delete the Rdata object)
  ...
  mod_S3bis <- data %>%
    group_by(uniqueID) %>%
    mutate(Active2 = -0.5 + as.numeric(mean(behave == 1 & which_behaviour == "Active") > 0)) %>%
    filter(which_behaviour == "Disp") %>%
