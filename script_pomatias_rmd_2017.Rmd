...
p1 <- newdata %>%
  ...
  ggplot() +
  stat_lineribbon(aes(x = Density, y = .value), .width = 0.95, point_interval = median_hdi, fill = "grey90",alpha=0.9) +
...
p3 <- newdata %>%
  ...
  stat_eye(aes(x = Disp, y = invlogit(value)), 
           .width = 0.95,interval_size=1,point_size=3,point_interval = median_hdi,alpha=0.9) +
...
as_tibble(ranef(mod, summary = FALSE)$ID[, , "scalelogshellmass_Intercept"]) %>%
  ...
  stat_eye(aes(x = name, y = exp(value * attr(data$scale_logshellmass, "scaled:scale"))), 
           .width = 0.95,interval_size=1,point_size=3,point_interval = median_hdi,alpha=0.9) +
  scale_x_discrete("") +
  scale_y_continuous("Predicted relative shell mass (posterior)", breaks = c(0.95, 1, 1.05)) +
  theme_bw()+
  theme(legend.position = "none",panel.border = element_blank(),axis.line = element_line(colour = "black"))
```
